...
## [2026-07-22] v2.0.1b — Path Architecture Fix, routing.log Only

### Changed

- **Separated MEMORY.md from sub-doc paths** — `OFFICIAL_MEMORIES_DIR` (`memories/`, Hermes official) and `SUBDOC_DIR` (`memory/`, custom sub-docs) are now distinct variables. Previously both were conflated under `MEMORIES_DIR`.
- **`MEMORY_MD` now points to `memories/MEMORY.md`** — aligns with Hermes official file location.
- **Routing logs written exclusively to `memory/routing.log`** — removed `## 路由日志` section from `update_memory_index()`. Renamed to `init_memory_index()` — only creates static headers (`## 规则`, `## 路由目标`) if MEMORY.md is absent. Idempotent.
- **Index paths use `../memory/` relative prefix** — since MEMORY.md lives in `memories/`, sub-doc paths in the index now use `../memory/{filename}` to resolve correctly from that directory.
- **Updated README directory tree** — `memory/` sub-docs reflect current routing categories: CREDENTIALS, infrastructure, tech-ref, dev-log, miscellaneous.

### Fixed

- **Sub-doc routing wrote to `memory/MEMORY.md` instead of `memories/MEMORY.md`** — broken `MEMORY_MD` path. Now correctly targets the Hermes official file.
- **Index entries used `memory/` prefix from `memories/` directory** — relative path resolved to `memories/memory/` (wrong). Fixed to `../memory/`.

---

## [2026-07-21] v2.0.1 — Routing Log, LLM Endpoint Switch, Code Cleanup

### Added

- **`routing.log`** — Timestamped one-line log per route call (category, method, elapsed time). Helps diagnose classification accuracy and LLM latency without noisy debug output.
- **`__version__ = '2.0.0'`** — Version tag in `server.py` and `intent_classifier.py` for runtime identification.
- **`CATEGORY_ALIAS`** — Mapping for hyphenated category names (e.g., `tech-ref` → `techref`) to handle input from intent_classifier module that uses hyphens.
- **`reasoning_content` fallback** — Thinking models that return `content=None` now fall back to `reasoning_content` field for classification.

### Changed

- **LLM classifier endpoint** — Switched from `10.10.4.8/Qwen3.6-27B` to `10.10.4.9/Qwen3.5-9B-AWQ` for faster, lighter classification.
- **`reasoning_effort='none'`** — Disabled thinking mode on SGLang/Qwen3.5 models to get direct JSON output without internal reasoning overhead.
- **`classify_intent()` return type** — Changed from `str` to `tuple` returning `(category, method)` where method is `'llm'` or `'keyword'`. Accurately reflects whether LLM or keyword fallback was used.
- **`classify_method` label** — No longer guesses fallback based on output category. Now reports the actual path taken by `classify_intent()`.

### Fixed

- **Unused `_last_classify_method` global variable** — Removed. Method tracking now done via tuple return value.
- **`classify_method` inaccuracy** — Previously labeled everything as "llm" or guessed fallback from result category. Now accurately tracks actual classification path.
...
3. **Release package** — Standardized installable package (tarball + install script) for easy deployment to new profiles
