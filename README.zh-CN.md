...
│   └── USER.md                 # 用户画像（注入系统提示）
│
└── memory/                      # 子文档（memory-routing 路由写入）
    ├── CREDENTIALS.md          — API密钥、密码、凭证
    ├── infrastructure.md       — IP、端口、服务部署
    ├── tech-ref.md             — API格式、部署指南
    ├── dev-log.md              — 开发变更、调优记录
    ├── miscellaneous.md        — 非技术类兜底
    └── routing.log             — 带时间戳的路由审计日志
```
...
1. Clone 仓库到 `~/.hermes/profiles/<name>/plugins/memory-routing/`
2. 安装 Python 依赖（`mcp[fastmcp]`、`requests`）
3. 自动从目标 profile 的 `model.*` 提取 LLM 配置（provider、model、base_url、api_key）
4. 在 `config.yaml` 中注册 MCP server 并注入环境变量
5. 验证 server.py 语法和配置条目
...
完整部署指南（卸载、排错、AGENTS.md 集成）见 [deploy/SKILL.md](deploy/SKILL.md)。

## 下一步计划（v2.1.0）

1. **模型选择** — 按任务可配置模型（轻量模型快速路由，重型模型处理边界情况）
2. **思考模式开关** — 运行时切换 `reasoning_effort`，按需启用/禁用 thinking 模型
